...
There are three main types of operations we are interested in, one is querying for edge data getEdge(v,w), another one is for fanout getFanout(v), another for getIntersection(v,w) (there is a tentative random walk one that I am yet to even define, more on that after I get the basics done).  Besides these, there are also update operation but in my reduced version of the graph store, the only  update is putEdge(Edge e). 
